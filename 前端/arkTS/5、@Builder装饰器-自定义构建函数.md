...
自定义构建函数的参数传递有[按值传递](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-builder-V5#%E6%8C%89%E5%80%BC%E4%BC%A0%E9%80%92%E5%8F%82%E6%95%B0)和[按引用传递](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-builder-V5#%E6%8C%89%E5%BC%95%E7%94%A8%E4%BC%A0%E9%80%92%E5%8F%82%E6%95%B0)两种，均需遵守以下规则：

- 参数的类型必须与参数声明的类型一致，不允许undefined、null和返回undefined、null的表达式。
- 在@Builder修饰的函数内部，不允许改变参数值。
- @Builder内UI语法遵循[UI语法规则](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-create-custom-components-V5#build%E5%87%BD%E6%95%B0)。
...
按引用传递参数时，如果在@Builder方法内调用自定义组件，ArkUI提供[$$](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-two-way-sync-V5)作为按引用传递参数的范式。
...
调用@Builder装饰的函数默认按值传递。当传递的参数为状态变量时，状态变量的改变不会引起@Builder方法内的UI刷新。所以当使用状态变量的时候，推荐使用[按引用传递](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-builder-V5#%E6%8C%89%E5%BC%95%E7%94%A8%E4%BC%A0%E9%80%92%E5%8F%82%E6%95%B0)。
...
这里传递的是一个 string 类型的值变量。使用按值传递的方式，在`@ComponentV2`装饰器修饰的自定义组件里配合使用`@ObservedV2`和`@Trace`装饰器可以实现刷新UI功能。

**正例**

在`@ComponentV2`装饰中，只有使用`@ObservedV2`修饰的ParamTmp类和@Trace修饰的count属性才可以触发UI的刷新。
...
在`@ComponentV2`装饰的自定义组件中，使用简单数据类型不可以触发UI的刷新。
...
创建私有的@Builder方法，在Column里面使用`this.builder()`方式调用，通过aboutToAppear生命周期函数和按钮的点击事件改变builder_value的内容，实现动态渲染UI。
...
创建全局的@Builder方法，在Column里面使用`overBuilder()`方式调用，通过以对象字面量的形式传递参数，无论是简单类型还是复杂类型，值的改变都会引起UI界面的刷新。
...
### 使用全局和局部的@Builder 传入 customBuilder 类型
...
### @Builder 函数联合 `V2` 装饰器使用

使用全局@Builder和局部@Builder在`@ComponentV2`修饰的自定义组件中调用，修改相关变量触发UI刷新。
