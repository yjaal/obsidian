...
这里整体逻辑和之前处理客户端连接基本类似，也是先分配适当大小的缓存，然后通过一个循环来处理每个 `ready` 的读请求。同时这里同样控制了每次循环的次数（默认 16 次）以及每个 `channel` 读取数据量的大小。这里控制次数的原因很简单，就是一个从 `Reactor` 上面会注册多个 `channel`，`Netty` 不可能在一个 `channel` 上无限制的处理下去，需要进行分摊，同时还有异步任务需要处理。
