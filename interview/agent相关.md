...
这是 RAG 的“地基”，目标是把原始文档处理成可高效检索的格式。

1.  **文档加载与解析**：从 PDF、Word、HTML、数据库等来源读取数据。难点在于处理复杂版面（如表格、多栏排版），通常需要版面分析工具。
2.  **文本分块（Chunking）**：将长文档切成语义完整的小块。常用策略有固定大小、递归分块、语义分块等，并设置 10%-25%的重叠（Overlap）防止信息被切断。
3.  **向量化（Embedding）**：用 Embedding 模型将每个 Chunk 编码成向量。中文场景常用 BGE、M 3 E 等模型。
4.  **向量存储**：将向量和原始文本、元数据（来源、时间等）存入向量数据库（如 Milvus、FAISS、Pinecone）。

### 🔍 二、在线检索阶段：找到相关信息

用户提问后，系统需要快速、准确地从知识库中召回最相关的内容。

1.  **Query 理解与改写**：对用户原始问题做处理，包括：
    *   **改写**：同义扩展、指代消解、术语对齐。
    *   **拆解**：将复杂问题拆成多个子问题。
    *   **意图识别**：判断问题类型，决定检索策略。
2.  **混合检索（Hybrid Search）**：并行执行两路召回。
    *   **向量检索**：语义匹配，擅长处理同义、泛化表达。
    *   **关键词检索（BM 25）**：精确匹配，擅长处理数字、术语、专有名词。
3.  **结果融合**：将两路结果合并。常用** RRF（倒数排名融合）** 算法，无需调权，对异常值不敏感。
4.  **重排序（Rerank）**：用 Cross-Encoder 模型对初筛结果做精细打分，将最相关的文档顶到最前面。这是提升精度的关键一步。
5.  **上下文组装**：将重排后的 Top-K 文档片段，按一定格式拼接到 Prompt 中，准备送入 LLM。

### 🤖 三、生成阶段：LLM 合成答案

1.  **Prompt 构建**：将系统指令、用户问题、检索到的上下文、输出格式要求组合成最终 Prompt。
2.  **LLM 生成**：模型基于上下文生成答案。关键约束是**必须基于检索结果回答**，无答案时明确告知，禁止编造。
3.  **引用标注**：在答案中标注信息来源，增强可信度。

### 🔄 四、优化与评估环节

RAG 不是一次性链路，需要持续迭代。
*   **优化方向**：
    *   **检索侧**：优化分块策略、引入混合检索、增加 Rerank。
    *   **生成侧**：优化 Prompt、加入 Few-shot 示例、约束输出格式。
    *   **Query 侧**：增加改写、扩展、拆解。
*   **评估体系**：
    *   **检索指标**：召回率、命中率、MRR。
    *   **生成指标**：准确率、忠实度（Faithfulness）、完整性。
    *   **端到端指标**：任务完成率、用户满意度。



## 如何区分 prompt 的问题还是模型能力问题

**核心原则是** 先穷尽 Prompt，再怀疑模型。具体用四步实验定位：

**第一步，检查信息充分性**。检索结果里有没有答案？如果没有，那是检索问题，不是 Prompt 也不是模型。

**第二步，Prompt 消融实验**。加约束、加 Few-shot、加 CoT、加格式说明，看问题是否解决。如果解决了，就是 Prompt 问题。

**第三步，模型替换实验**。换更强的模型测试。如果强模型能解决、弱模型不能，就是模型能力不足。

**第四步，人工上限测试**。人工基于同样上下文写答案，如果人工能做好而模型做不好，就是模型能力不足；如果人工也做不好，就是任务定义或信息问题。

**核心认知**：大部分"模型能力不足"的抱怨，实际上是 Prompt 问题。因为 Prompt 优化的成本是换模型的 1%，但能解决 80%的问题。所以**先用实验确认根因，再决定投入方向**。


## 什么是 Lost in the Middle？

这是大模型长上下文处理中的一个经典问题——当上下文很长时，模型对开头和结尾的信息关注度最高，中间部分的信息容易被忽略。

**在 RAG 中，它的危害很大**：你召回了 10 个文档片段，如果最相关的恰好排在中间，模型可能根本没用到它，导致答案片面或错误。

**缓解策略有三层**：
1. **关键信息前置**：把最相关的文档放在最前面。
2. **重排序+截断**：用 Rerank 把最相关的顶到前面，只取 Top-3~5，减少中间区域的信息量。
3. **上下文压缩**：对长文档做摘要，缩短上下文长度。
**核心认知**：上下文不是越长越好。**“把最相关的信息放在模型最容易看到的位置”**，比“塞更多信息”更重要。